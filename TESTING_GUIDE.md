...
|---|---|---|
| Domain logic in `todo_store.hpp` | Catch2 unit test in `tests/todo_store_test.cpp` | C++ |
| New bridge method in `bridge.hpp` | Add the method to `test-server/server.ts` (the Bun mock). Proxy tests cover the protocol automatically. | Bun |
| UI behavior changed | Playwright e2e test in `e2e/todo-lists.spec.ts` | E2E |
| Nothing visible changed | You probably don't need a new test | — |

You almost never need to touch the smoke tests. They verify Qt renders React at all — not your app's behavior.
...
| **Playwright e2e** (`test-e2e`) | UI + backend integration is broken. Could be React, could be the bridge, could be the server. |
| **CDP smoke** (`test-smoke`) | Qt isn't rendering the React app at all. Check that `xmake build desktop` succeeds and the app launches. |
...
| E2E (Playwright) | `xmake run test-e2e` | ~5s |
| CDP smoke | `xmake build desktop && xmake run test-smoke` | ~15s |
| All (Catch2 + Bun + e2e) | `xmake run test-all` | ~10s |
...
**CDP smoke** launches the real Qt desktop app and connects via Chrome DevTools Protocol. It's slower and can be flaky (GPU, window manager). Good for CI, don't gate on it locally.
...
| Smoke tests fail | Desktop app didn't build. Run `xmake build desktop` first. |
| Smoke tests flaky | GPU/window manager issues. These tests are inherently less stable. |

## How Do I Add a New Test?

### Catch2 — testing domain logic

Add a test case to `tests/todo_store_test.cpp`. Direct C++ — no mocking, no setup:
...
Add a test to `tests/bridge_proxy_test.ts`. Each test spins up its own WebSocket server:
...
Add a test to `e2e/todo-lists.spec.ts`. Drives a real browser against the full stack:
...
Run: `xmake run test-e2e`

### Bun mock server — keeping it in sync

When you add a new bridge method, add the corresponding handler to `test-server/server.ts` so e2e tests work against both the C++ server and the Bun mock:
...
```
Catch2           Bun              Playwright e2e         CDP smoke
  │                │                   │                     │
  ▼                ▼                   ▼                     ▼
TodoStore     WsBridge Proxy     React + C++ server     Real Qt app
(pure C++)    (protocol only)    (full integration)     (renders at all?)
```
