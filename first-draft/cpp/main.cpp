...
int main(int argc, char* argv[]) {
    ...
    app.setApplicationName("Qt Web Shell Example");
    app.setWindowIcon(QIcon(":/Skykit.ico"));

    // Dark palette — prevents white flash on first frame.
    // Palette is immediate (stylesheets are applied lazily, too late).
    ...
    auto* profile = new QWebEngineProfile("QtWebShellExample", &app);
    ...
    QMainWindow window;
    window.setWindowTitle("Qt Web Shell Example");
    ...
    window.move((geo.width() - 1280) / 2 + geo.x(),
                (geo.height() - 800) / 2 + geo.y());
    ...
    QObject::connect(aboutAction, &QAction::triggered, &window, [&window]() {
        ...
        aboutBox.setText("Qt Web Shell Example\nA minimal Qt + WebEngine + React shell.");
        aboutBox.setIconPixmap(QIcon(":/Skykit.ico").pixmap(64, 64));
        aboutBox.exec();
    });
    ...
    logo->setPixmap(QPixmap(":/Skykit.png").scaled(128, 128, Qt::KeepAspectRatio, Qt::SmoothTransformation));
    ...
    // Without this, Windows DWM flashes a white frame before Qt gets to paint.
